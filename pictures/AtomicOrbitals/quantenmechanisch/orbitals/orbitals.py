#!/usr/bin/env python3
### https://github.com/sidstuff/orbital
...
try:
    ...
    #print("can not load override.conf")
    pass
...
n = int(args[0] or input(f"n (default {defaults[0]}) = ") or defaults[0])    # If a value is False, moves onto the next.
l = int(args[1] or input(f"l (default {defaults[1]}) = ") or defaults[1])    # defaults having a non-string 0 doesn't matter,
m = int(args[2] or input(f"m (default {defaults[2]}) = ") or defaults[2])    # since it's the last value.

dots = int(args[3] or input(f"Number of dots (default {defaults[3]}): ") or defaults[3])
bgcolor = (args[4] or input(f"Background color (default  {defaults[4]}):") or defaults[4]) 
colormap = eval(args[5] or input(f"Change colormap to (optional):") or defaults[5])
mode = (args[6] or input(f"View/Draw/Animate [V/D/A] ({defaults[6]}): ")).strip().lower()
...
if mode in ["view", "v"]:
    ...
    size = defaults[11]
    elevation = float(args[7] or input(f"Elevation in degrees (default {defaults[7]}): ") or defaults[7])
    if mode in ["draw", "d"]:
        mode = 'drawings'
        azimuth = float(args[8] or input(f"Azimuth in degrees (default {defaults[8]}): ") or defaults[8])
        format = (getformat(0) or input(f"Format (default {defaults[9][0]}): ") or defaults[9][0]).strip().lower()
    else:
        mode = 'animations'
        azimuth = float(args[8] or input(f"Initial azimuth in degrees (default {defaults[8]}) : ") or defaults[8])
        format = (getformat(1) or input(f"Format [mp4/webm/gif] (default {defaults[9][1]}): ") or defaults[9][1]).strip().lower()
        duration = int(args[10] or input(f"Duration in sec (default {defaults[10]}): ") or defaults[10])	
        fps = defaults[14]
